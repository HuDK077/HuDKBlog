...
class ApiFileController extends Controller
{
     ...
     * @description 获取文件接口
     * @method POST
     * @url http://xx.com/admin/file/getImageFile
     * @header token 必选 string 设备token
     * @param file_id 必选 string 文件ID
     * @return{"error_code": 2001,"data": {"image": "图片base64编码"},"message": "success"}
     * @return_param error_code int 返回码
     * @return_param message string 返回说明
     ...
     * @header token 必选 string 设备token
     * @param file 必选 文件 文件
     * @param type 非必选 array 允许上传的文件类型application('pdf''xlsx''docx'),video('rmvb''rm''flv''mp4''3gp','wmv','avi'),image('jpg''png''gif''jpeg')
     * @return
     * @return_param error_code int 返回码
     * @return_param message string 返回说明
     * @return_param name string 用户名称
        ...
        if (!$temp_file) {
            ...
            switch ($disk) {
                case "local": //本地
                    if ($file->move(storage_path() . $path, $fileName)) {
                        File::create($data);
                        $res['img'] = env('APP_URL') . '/api/file/getFile?file_id=' . $file_Id;
                    } else {
                        return apiResponse(2005, [], '上传失败');
                    }
                    break;
...
//                $res_data = ['Content-Type' => $file->mime_type];
            ...
            case "qiniu":
                ...
                $qiniu = \Storage::disk('qiniu');
                $file_exists = $qiniu->exists($temp_path);
                if (!$file_exists) {
                    return apiResponse(2009, [], 'CDN无此文件');
                }
                $file_url = $qiniu->url(['path' => $file->file_path, 'domainType' => 'default']);
                return apiResponse(2001, $file_url);
                break;
            default:
    ...
    public function getUploadConfig()
    {
        ...
        $expires = 3600 * 24;
        $policy = [
            "persistentOps" => "avthumb/mp4/ab/160k/ar/44100/acodec/libfaac/r/30/vb/2400k/vcodec/libx264/s/1280x720/autoscale/1/stripmeta/0",
            "persistentNotifyUrl" => env("APP_URL") . "/api/file/qiniuNotify",
            "persistentPipeline" => "taoduwang",
            "saveKey" => "\${fname}",
        ];
        $upToken_video = $auth->uploadToken($bucket, null, $expires, $policy, true);
        $upToken_image = $auth->uploadToken($bucket, null, $expires, null, true);
        $data = [
            'region' => env('QINIU_DEFAULT_REGION'),
            'upToken_video' => $upToken_video,
            'upToken_image' => $upToken_image,
        ];
        return apiResponse(2001, $data);
    }
    ...
    public function qiniuNotify(Request $request){
        ...
        try{
            foreach ($request["items"] as $item){
                if ($item["code"] == 0){
                    $qiniu->delete($request["inputKey"]);
                    $qiniu->move($item["key"], $request["inputKey"]);
                }
            }
        }catch(\Exception $exception){
            info([__METHOD__,__LINE__,$exception->getMessage()]);
        }
    }
}
