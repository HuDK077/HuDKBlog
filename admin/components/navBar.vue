<template>
  <div class="navbar">
    <div class="bar">
      ...
      <div class="right">
        <el-button @click="showError(true)" v-if="errorLogs.length && openErrorLog" type="text" class="error_message" icon="app_debug">错误</el-button>
        <!-- mqtt连接 -->
        <span class="mqtt" v-if="hasMqtt">
          <el-popconfirm confirmButtonText="好的" cancelButtonText="不用了" :icon="connected ? 'app_weilianjie' : 'app_yilianjie'" :iconColor="connected ? '#f56c6c' : '#67c23a'" :title="connected ? '确定断开mqtt连接吗' : '确定连接mqtt服务吗'" v-if="!connecting" @confirm="switchStatus">
            <el-button slot="reference" type="text" :icon="connected ? 'app_yilianjie' : 'app_weilianjie'"></el-button>
          </el-popconfirm>
