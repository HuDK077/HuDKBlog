...
export default {
  ...
  methods: {
    ...
    filterNode(value, data) {
      ...
      return data.name.indexOf(value) !== -1 || data.slug.indexOf(value) !== -1;
    },
    // 列表被点击
    handleNodeClick(e) {
      console.log(e);
      ...
      this.$apis.getRM({ id })
        .then(res => {
          ...
          if (error_code == 2001) {
            ...
            // this.treeActivePages = this.$clone(menu_ids);
    ...
    makeWidgetList() {
      if (this.widgets.length) {
        let widgets = this.$clone(this.widgets)
        let tempPages = this.activePages
        let tempChoosePages = this.choosePages;
        let widgetTree = []
        // 先分两组,页面选中的和页面未选中的
        let activePages = []
        let disabledPages = []
    ...
    // 保存信息
    saveData() {
      let { activePerlission, activePages, activeWidgets } = this

      this.$apis.updateRM({ role_id: activePerlission.id, menu_ids: activePages, widget_ids: activeWidgets })
        .then(res => {
          console.log(res.data)
          let { error_code, data, message } = res.data;
          if (error_code == 2001) {
