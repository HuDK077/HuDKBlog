...
export default function ({ $axios, store, env, next, error, route }) {
  ...
  $axios.interceptors.request.use((request) => {
    ...
    const store_id = store.getters['auth/store_id']
    if (token) {
    ...
    if (request.method == 'post' && (request.headers["Content-Type"] && request.headers["Content-Type"].includes("application/json"))) {
      // console.log(request);
      if (!request.data || (request.data && !request.data.hasOwnProperty("store_id"))) {
        request.data = { ...request.data, store_id }
        request.data = QS.parse(QS.stringify(request.data))
      }
    } else if (request.method == 'get') {
      if (request.params || (request.params && !request.params.hasOwnProperty("store_id"))) {
        request.params = { ...request.params, store_id }
        request.params = QS.parse(QS.stringify(request.params))
      }
    }
    // window.req = request;
    return request
